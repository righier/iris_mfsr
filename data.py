...
class ImageSequenceDataset(Dataset):
  def __init__(self, basedir, xnames, yname, dirs, single_image):
  ...
  def load_image(self, dir, name):
    ...
    if self.grayscale: img = TF.to_grayscale(img)
  ...
  def __getitem__(self, idx):
    dir = self.dirs[idx]
    if self.single_image: X = self.load_image(dir, xnames[3])
    else: X = torch.stack([self.load_image(dir, name) for name in xnames]).permute(1,0,2,3)
    Y = self.load_image(dir, self.yname)
    return X, Y

def VimeoDataset(basedir, scale=2, single_image=False, train=True, patches=True):
  ...
  dirs = [os.path.join(dir, patch) for dir in dirs for patch in range(8)]
  basedir = os.path.join(basedir, 'sequences')
  return ImageSequenceDataset(basedir, xnames, yname, dirs, single_image)

def FaceDataset(basedir, scale=2, single_image=False, train=False):
...
def EyesDataset(basedir, scale=2, single_image=False, train=False):
